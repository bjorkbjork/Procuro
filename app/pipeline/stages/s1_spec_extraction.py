...
def fetch_page_html(url: str, proxy_country: str | None = None) -> str:
    with BrowserSession(proxy_country=proxy_country) as s:
...
def extract_specs(url: str) -> SourceProduct:
    ...
    html = fetch_page_html(url, proxy_country=source.proxy_country)
