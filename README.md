...
Autonomous agent that scrapes product specs from retailer websites (Kogan, Kmart, etc.), finds matching suppliers on GlobalSources and Alibaba, negotiates FOB prices via email, and logs results to Google Sheets.
...
A [Browserbase](https://www.browserbase.com/) account for cloud browser sessions. Residential proxies are used for AU geolocation to bypass bot detection on retailer sites.
