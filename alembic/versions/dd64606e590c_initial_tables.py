...
def upgrade() -> None:
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.CheckConstraint("platform IN ('alibaba', 'globalsources')", name='check_platform'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('supplier_threads',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    sa.CheckConstraint("state IN ('NEW', 'OUTREACH_SENT', 'AWAITING_REPLY', 'SPEC_CHECK_PASS', 'SPEC_CHECK_FAIL', 'NEGOTIATING', 'FINAL_PRICE_LOGGED', 'CLOSED')", name='check_state'),
    ...
    sa.ForeignKeyConstraint(['thread_id'], ['supplier_threads.id'], ),
    sa.PrimaryKeyConstraint('id'),
